"""Entry point for experiment 05 — score 0-5 con rúbrica binaria 5 dimensiones.

Usage:
    cd <repo root>
    # Full run (3 models × 4 registers × n=5):
    python -m experiments.exp_05_rubrica.run

    # Filtered (only Kimi):
    python -m experiments.exp_05_rubrica.run --models kimi-k2.6

    # Only control (validate parser):
    python -m experiments.exp_05_rubrica.run --registers control --n 2 --workers 2

    # Specific registers:
    python -m experiments.exp_05_rubrica.run --registers control,hostil-correctivo

Registers: control, adulacion, hostil-correctivo, hostil-combinado.
Logs go to runs/exp_05_rubrica/<timestamp>/.
"""
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run experiment 05 — score 0-5 con rúbrica binaria 5 dimensiones."
    )
